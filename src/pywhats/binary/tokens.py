...
"""Token dictionaries for the binary stanza codec.

Complete single-byte and double-byte token tables for the current
WhatsApp Web wire format. A single-byte token is one byte 1..235
indexing into SINGLE_BYTE_TOKENS. A double-byte token is a selector
(236..239 -> dict 0..3) followed by an index.
"""
...
NIBBLE_ALPHABET = "0123456789-.\x00\x00\x00\x00\x00"
HEX_ALPHABET = "0123456789ABCDEF"

SINGLE_BYTE_TOKENS: list[str] = [
    "",  # 0: LIST_EMPTY (reserved)
    "xmlstreamstart",
    "xmlstreamend",
    "s.whatsapp.net",
    "type",
...
_SINGLE_INDEX: dict[str, int] = {s: i for i, s in enumerate(SINGLE_BYTE_TOKENS) if s}
_DOUBLE_INDEX: dict[str, tuple[int, int]] = {}
for _dict_no, _entries in enumerate(DOUBLE_BYTE_TOKENS):
    for _idx, _s in enumerate(_entries):
        if _s:
            _DOUBLE_INDEX.setdefault(_s, (_dict_no, _idx))


def lookup_single(s: str) -> int | None:
    return _SINGLE_INDEX.get(s)


def lookup_double(s: str) -> tuple[int, int] | None:
    return _DOUBLE_INDEX.get(s)


def single_token_at(index: int) -> str:
    if 0 < index < len(SINGLE_BYTE_TOKENS):
        tok = SINGLE_BYTE_TOKENS[index]
        if tok:
            return tok
    raise ValueError(f"unknown single-byte token index {index}")


def double_token_at(dict_no: int, index: int) -> str:
    if not 0 <= dict_no < len(DOUBLE_BYTE_TOKENS):
        raise ValueError(f"unknown double-byte dictionary {dict_no}")
    entries = DOUBLE_BYTE_TOKENS[dict_no]
    if 0 <= index < len(entries) and entries[index]:
        return entries[index]
    raise ValueError(f"unknown double-byte token [{dict_no}][{index}]")
